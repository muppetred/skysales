...
| `<MASTER_NL>` | **MASTER SD Sales P1+P2 - NEUTRE NL (skysales)** | 43 | **R2 en néerlandais.** Copié de l'EN et traduit intégralement le 26/08/2026, registre de vouvoiement **`u`**. **C'est lui qu'on copie pour un deck NL.** |
...
8. **Langue selon le client** (EN / FR / NL). Demander avant de remplir. Ponctuation française si FR (espaces avant `: ; ! ?`, guillemets « »). Pas de tiret cadratin. **En NL** : vouvoiement **`u`** partout (jamais `je` / `jij`), séparateur de milliers en point (`1.500 €`, pas `1,500 €`), pas d'espace avant la ponctuation, jours abrégés en deux lettres (`ma di wo do vr`). Le filtre anti-AI couvre EN et FR (voir `references/writing-filter.md`) — **il n'a pas encore de section NL** : sur un deck néerlandais, faire relire le texte visible par un natif avant livraison, et le noter comme tel dans le rapport.
...
19. **Plus d'inputs, jamais plus de structure.** Le deck a un squelette fixe : le même nombre de pages, les mêmes sections, dans le même ordre, avec les mêmes zones. Ce squelette vient du master et ne se modifie jamais de sa propre initiative. Un input supplémentaire (mail, présentation, document, lien) sert à **remplir mieux** les zones existantes, jamais à ajouter une page, en supprimer une, en réordonner, ni à inventer une section « bonus ». **Seule exception : le commercial demande explicitement, pour un deck donné, d'en raccourcir la P2.** C'est une demande ponctuelle, jamais un défaut ni une version à proposer d'office : le deck suivant repart du squelette complet. Si du contenu d'une slide retirée est rapatrié dans le scope, le prix repasse par le calcul (`references/pricing.md`).
...
Master à copier : **fixé par la règle 10** (`<MASTER_EN>` / `<MASTER_FR>` / `<MASTER_NL>` pour un R2 selon la langue, `<DIGEST>` pour un R1) et validé par le préflight à chaque run.
...
**Le néerlandais déborde davantage encore** (constaté à la construction du master NL, 26/08/2026) — non pas parce que les phrases sont plus longues, mais parce que les **compounds** le sont : un seul mot soudé (`GEÏNTEGREERD SYSTEEM`, `EIGENAARSCHAP`, `concurrentiebenchmark`, `kalendervalidatie`) ne se coupe pas et sort du cadre d'un bloc. Deux réflexes : élargir avec `resize_element` (`width` seul) tant qu'il y a de la place à droite, et **sinon choisir un synonyme plus court plutôt que d'accepter le retour à la ligne** — `Mogelijke add-ons` au lieu de `Beschikbare add-ons`. Surveiller en particulier les **titres à deux lignes** : une ligne qui passe à trois fait grandir la boîte vers le bas et casse l'alignement de toute la page.
...
1. **La navigation latérale est en dur en anglais** — `diagnosis`, `sector signals`, `current state`, `priorities`, `strategic approach`, `platform roles`, `Cultural layer`, `content strategy`, `Pillar 01/02/03`, `team`. Environ 40 éléments sur une quinzaine de pages. Sur un deck français, les laisser produit un document bâtard. Les traduire à mesure, dans le lot de chaque page. **Exceptions à garder en anglais** (règle 9) : `social-os`, `way of working`, `always-on`, `kick-off`, `scope`, `pricing`, les cinq phases du Social OS™ et l'échelle `AWARENESS → RECURRENCE → RELEVANCE → PREFERENCE`.

   **Table NL arrêtée le 26/08/2026** (master `<MASTER_NL>`) : `diagnosis` → `diagnose` · `introduction` → `introductie` · `sector signals` → `sectorsignalen` · `current state` → `stand van zaken` · `priorities` → `prioriteiten` · `strategic approach` → `strategische aanpak` · `platform roles` → `platformrollen` · `Cultural layer` → `culturele laag` · `content strategy` → `content strategie` · `Pillar 01/02/03` → `Pijler 01/02/03` · `team` → `team` (identique). Les libellés de page suivent : `Starts from` → `Vertrekt van` · `Feeling` → `Gevoel` · `Where` → `Waar` · `Example content` → `Voorbeelden` · `Included` → `Inbegrepen`. **Piège vérifié :** `way of working` a d'abord été traduit en `manier van werken` puis remis en anglais — c'est une exception de la règle 9, elle vaut pour toutes les langues, pas seulement le français.
...
- Master : **fixé par son ID et par la langue** — `<MASTER_FR>` (R2 FR), `<MASTER_EN>` (R2 EN), `<MASTER_NL>` (R2 NL), `<DIGEST>` (Digest R1), tous dans le dossier `Template` (`<DOSSIER_TEMPLATE>`). Ne pas le redemander au lead ; le valider par le préflight à chaque run (règle 10).

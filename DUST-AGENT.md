...
19. **Plus d'inputs, jamais plus de structure.** Le deck a un squelette fixe : le même nombre de pages, les mêmes sections, dans le même ordre, avec les mêmes zones. Ce squelette vient du master et tu ne le modifies jamais de ta propre initiative. Un input supplémentaire — mail, présentation, document, lien — sert à **remplir mieux** les zones existantes, jamais à ajouter une page, en supprimer une, en réordonner, ni à inventer une section « bonus ». **Seule exception : le commercial te demande explicitement, pour un deck donné, d'en raccourcir la P2.** C'est une demande ponctuelle, jamais un défaut ni une version à proposer d'office : le deck suivant repart du squelette complet.
